...
const fetchNotices = async () => {
  ...
  try {
    ...
    console.log("Notices data:", res.data);
    setNotices(res.data);
  } catch (err) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Notices from your Head of Department</p>
          </div>
          {unreadCount > 0 && (
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            <p className="text-gray-500 text-xs font-medium">Total Notices</p>
        ...
        <div className="flex gap-2 bg-white rounded-2xl p-1.5 shadow-sm border border-gray-100 w-fit">
          {[
            ...
            <button
              ...
              className={`px-5 py-2 rounded-xl text-sm font-medium transition-all ${filter === tab.key ? "bg-emerald-600 text-white" : "text-gray-500 hover:text-gray-700"}`}
            >
              {tab.label}
            </button>
          ))}
            ...
            {filteredNotices.map((notice) => (
              <div
                ...
                className={`bg-white rounded-2xl shadow-sm border overflow-hidden transition-all
                  ${!notice.is_read ? "border-amber-200 ring-1 ring-amber-100" : "border-gray-100"}`}
              >
                {/* Notice Header */}
                <div
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                      ...
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${notice.recipient_type === "all" ? "bg-indigo-50 text-indigo-600" : "bg-purple-50 text-purple-600"}`}>
                        {notice.recipient_type === "all" ? "All Faculty" : "Addressed to you"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="px-6 py-5 border-t border-gray-100 bg-gray-50">
                    ...
                    <div className="mt-4 pt-3 border-t border-gray-200 flex items-center justify-between">
                      <p className="text-xs text-gray-400">
                        Sent on {formatDate(notice.created_at)}
                      </p>
                      {notice.is_read && (
